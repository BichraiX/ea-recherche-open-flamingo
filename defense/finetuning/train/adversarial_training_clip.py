...
from train.datasets import COCOFlickrDataset, ImageNetDataset, CIFAR100Dataset
...
def main(args):
    ...
    if args.dataset == 'imagenet':
        ...
        root=args.dataset_root,
            train=True,  # Use training set or set to False for validation
            transform=preprocessor_without_normalize,
            download=True  # Download CIFAR-100 if not available
        )
    ...
    if args.dataset == 'imagenet':
        ...
        dataset_eval = CIFAR100Dataset(
        root=args.dataset_root,  # Specify the CIFAR-100 root directory
    ...
    #torch.save(model, f'{args.output_dir}/checkpoints/final_model.pt')
...
def train_one_epoch(
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        if (step_total-1) % args.log_freq == 0:
            ...
            if (step_total-1) % (args.log_freq * 4) == 0:
        ...
        if args.save_checkpoints and (step_total % (args.steps // 10) == 0): #to be modified
...
if __name__ == '__main__':
    ...
    args.finetuned_model_name = f'{args.clip_model_name}_{args.pretrained}_{args.dataset}_{args.loss}_{args.dataset}_{args.experiment_name}'
